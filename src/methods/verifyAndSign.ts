...
function filterVersions(
  versions: OverrideVersionedType[] = [],
  specVersion: number
): RegistryTypes {
  return versions
    .filter(
    ...
    .reduce((result: RegistryTypes, { types }): RegistryTypes => objectSpread(result, types), {});
}
...
function getSpecHasher(
  ...
  return (
    knownTypes.hasher ||
    knownTypes.typesBundle?.chain?.[_chainName]?.hasher ||
    knownTypes.typesBundle?.spec?.[_specName]?.hasher ||
    null
  );
}
...
const initRegistry = async (
  ...
  registry.register(
    getSpecTypes(
      registry,
      registryInfo.chainName as any,
      registryInfo.runtimeVersion.specName as any,
      registryInfo.runtimeVersion.specVersion
    )
  );
  registry.setHasher(
    getSpecHasher(
