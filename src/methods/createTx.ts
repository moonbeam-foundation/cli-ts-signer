  ...
  if (txOpt.eraPeriod !== undefined) {
    ...
    if (!Number.isSafeInteger(txOpt.eraPeriod) || txOpt.eraPeriod < 4) {
      console.log(chalk.red(
        `❌ Era period must be an integer >= 4`
      ));
    ...
    if (!Number.isInteger(Math.log2(txOpt.eraPeriod))) {
      console.log(chalk.red(
        `❌ Era period must be a power of 2 (e.g., 512, 1024, 2048, 4096, 8192, 16384).`
      ));
    ...
    if (eraPeriod > 4096) {
      console.log(chalk.yellow(
        `⚠️  Using era period of ${eraPeriod} blocks (${(eraPeriod * 12 / 3600).toFixed(1)} hours on Moonbeam).\n` +
